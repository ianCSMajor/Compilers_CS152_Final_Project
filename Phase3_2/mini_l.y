...
statement: int_var ASSIGN expression {stringstream ss; ss << $3.code << "= " << $1.ret << ", "  << $3.ret << "\n"; $$.code = strdup(ss.str().c_str());}
  ...
  | IF bool_exp THEN statements ENDIF {$$.code = (char*)"";}
...
bool_exp: nots expression comp expression {stringstream ss; $$.ret = strdup(var_generator()); ss << $1.code << $2.code << $4.code << ". " << $$.ret << "\n" << $3.code << " " << $$.ret << ", " << $2.ret << ", " << $4.ret << "\n"; $$.code = strdup(ss.str().c_str());}
  | nots L_PAREN expression comp expression R_PAREN {stringstream ss; $$.ret = strdup(var_generator()); ss << $1.code << $3.code << $5.code << ". " << $$.ret << "\n" << $4.code << " " << $$.ret << ", " << $3.ret << ", " << $5.ret << "\n"; $$.code = strdup(ss.str().c_str());}
